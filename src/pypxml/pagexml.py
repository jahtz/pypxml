...
class PageXML:
    ...
    def __init__(self, 
        ...
        """
        self.creator: str = "pypxml" if creator is None else creator
        self.__height: int = int(height)
        self.__width: int = int(width)
        self.__filename: str = filename if isinstance(filename, str) else Path(filename).name
        self.__xml: Optional[Path] = None if xml is None else Path(xml).absolute()
        
        if isinstance(created, datetime):
            self.__created: str = created.isoformat()
        ...
        if isinstance(last_change, datetime):
            self.__last_change: str = last_change.isoformat()
        elif isinstance(last_change, str):
            self.__last_change: str = last_change
        else:
    ...
    def width(self, value: Union[str, int]) -> None:
        ...
        if isinstance(value, str):
            value = int(value)
        if value < 0:
            raise ValueError("The width can not be negative")
        self.__width = value
    ...
    def attributes(self, attributes: Optional[dict[str, str]]) -> None:
        ...
        if "imageFilename" in attributes:
            self.filename = attributes["imageFilename"]
        if "imageHeight" in attributes:
            self.height = attributes["imageHeight"]
        if "imageWidth" in attributes:
            self.width = attributes["imageWidth"]
    ...
    def find_by_type(self, pagetype: Union[PageType, list[PageType]], depth: int = 0, 
        ...
        for element in self.__elements:
            if element.pagetype in pagetype:
                if not attributes or all(element[str(k)] == str(v) for k, v in attributes.items()):
    ...
    def set_reading_order(self, reading_order: list[str], sync: bool = True) -> None:
        """
        Set the reading order of the PageXML.
        Args:
            reading_order: A list of region id's (Does not check for valid id's!).
    ...
    def sort_reading_order(self, base: Literal["minimum", "maximum", "centroid"] = "minimum",
        ...
        def sort_key(obj: PageElement):
            if not obj.is_region():
                return (0, 0)
            elif (coords := obj.get_coords()) is not None and "points" in coords:
        ...
        self.__reading_order = list([e["id"] for e in sorted_elements if e.is_region() and "id" in e])
        if sync:
            self.apply_reading_order()
