...
class PageElement:
    ...
    def __init__(self, pagetype: PageType, parent: Union["PageXML", Self], **attributes: str) -> Self:
        ...
        self.__attributes = {str(k): str(v) for k, v in attributes.items() if v is not None}
    ...
    def is_region(self) -> bool:
        ...
        return PageType.is_region(self.__pagetype)
    ...
    def get_baseline(self) -> Optional[Self]:
        """
        Get the baseline element.
        Returns:
            The PageType.Baseline element of the current object if it exists.
        """
        if (baseline := self.find_by_type(PageType.Baseline, depth=0)):
            return baseline[0]
        return None
    
    def get_text(self, index: Optional[int] = None, depth: int = -1) -> Optional[str]:
        """
        Find a PageType.TextEquiv element and return the text of its child PageType.Unicode element.
        Args:
            index: Should be specified if multiple TextEquiv. Defaults to None.
            depth: Depth level of the search for PageType.TextEquiv elements. 
                   `0`: Search only the current level. 
                   `-1`: Search all levels recursively (no depth limit). 
                   `>0`: Limit the search to the specified number of levels deep.
        Returns:
        ...
        """
        weight = {PageType.TextRegion: 5, PageType.TextLine: 4, PageType.Word: 3, PageType.Glyph: 2, 
                  PageType.GraphemeGroup: 1, PageType.Grapheme: 0, PageType.NonPrintingChar: 0}
        text_equivs = self.find_by_type(PageType.TextEquiv, depth, index=index)
        if len(text_equivs) > 0:
            text_equivs.sort(key=lambda e: weight[e.parent.pagetype])
            for text_equiv in text_equivs:
                if uc := text_equiv.find_by_type(PageType.Unicode, depth=0):
                    return uc[0].text
        return None
